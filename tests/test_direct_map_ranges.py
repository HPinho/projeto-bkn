#!/usr/bin/env python3
"""Guardrails do direct-map construído a partir do snapshot de boot."""
...
class DirectMapRangesTests(unittest.TestCase):
    ...
    def test_builder_walks_variable_size_boot_descriptors(self):
        self.assertIn("descriptor_size < 40", self.ranges)
        self.assertIn("offset += descriptor_size", self.ranges)
        self.assertIn("address as *const BootMemoryDescriptor", self.ranges)

    def test_builder_uses_baken_memory_policy_before_mapping(self):
        self.assertIn("boot_memory_descriptor_is_direct_map_wb", self.ranges)
    ...
    def test_direct_map_is_writable_global_and_nx(self):
        self.assertIn("X86_PTE_PRESENT | X86_PTE_WRITABLE | X86_PTE_GLOBAL | X86_PTE_NX", self.ranges)
    ...
    def test_range_builder_does_not_activate_cr3_or_firmware(self):
        code = "\n".join(line.split("//", 1)[0] for line in self.ranges.splitlines())
        for token in ("__write_cr3", "write_cr3", "__invlpg", "__wrmsr", "IA32_PAT", "ExitBootServices", "AllocatePages", "EFI_", "uefi_", "Efi"):
            self.assertNotIn(token, code)

    def test_graph_root_does_not_materialize_direct_map(self):
        self.assertIn("import kernel::memory::direct_map_ranges::*;", self.main)
        self.assertNotIn("direct_map_build_from_boot_ranges(", self.main)
