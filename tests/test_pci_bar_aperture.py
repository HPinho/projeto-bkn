...
class PciBarApertureContracts(unittest.TestCase):
    ...
    def test_publication_is_write_once_and_idempotent_only_for_same_measurement(self):
        body = self._body(APERTURE, "pci_bar_aperture_publish")
        self.assertIn("entry.bdf == bdf && entry.bar_index == bar_index", body)
    ...
    def test_publication_rejects_nonsensical_apertures(self):
        body = self._body(APERTURE, "pci_bar_aperture_publish")
    ...
    def test_registry_contains_metadata_not_ownership(self):
    ...
    def test_layout_consumes_registry_without_triggering_measurement(self):
