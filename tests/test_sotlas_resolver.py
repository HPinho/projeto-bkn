...
class SotlasResolverTests(unittest.TestCase):
    ...
    def test_build_modular_compiles_and_links_kernel_objects(self):
        ...
        self.assertEqual(len(result["compiled_objects"]), 14)
        self.assertEqual(len(result["generated_sources"]), 13)
        self.assertEqual(len(result["generated_headers"]), 13)
        self.assertTrue(all(Path(path).is_file() for path in result["generated_headers"]))
        self.assertEqual(len(result["generated_interfaces"]), 13)
