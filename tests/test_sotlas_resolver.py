...
class SotlasResolverTests(unittest.TestCase):
    ...
    def test_build_modular_compiles_kernel_objects(self):
        ...
        self.assertEqual(len(result["compiled_objects"]), 24)
        self.assertEqual(len(result["generated_sources"]), 23)
        self.assertEqual(len(result["generated_headers"]), 23)
        self.assertTrue(all(Path(path).is_file() for path in result["generated_headers"]))
        self.assertEqual(len(result["generated_interfaces"]), 23)
