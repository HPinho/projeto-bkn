...
class DmaContractTests(unittest.TestCase):
    ...
    def test_df8c_has_only_certified_typed_allocation_callers(self):
        ...
        self.assertEqual(
            sorted(callers),
            ["kernel/src/drivers/ahci_block_read.sotlas","kernel/src/drivers/ahci_runtime.sotlas","kernel/src/drivers/nvme.sotlas","kernel/src/drivers/xhci_context.sotlas","kernel/src/drivers/xhci_descriptor.sotlas","kernel/src/drivers/xhci_runtime.sotlas"],
            "DF-8c integration must remain limited to certified NVMe/AHCI callers plus xHCI runtime, context, and descriptor buffers"
        )
