#!/usr/bin/env python3
"""Guardrails do DMA físico pós-cutover, incluindo DF-8a/DF-8b/DF-8c."""
...
class DmaContractTests(unittest.TestCase):
    ...
    def test_df8c_has_only_certified_typed_allocation_callers(self):
        ...
        self.assertEqual(
            sorted(callers),
            ["kernel/src/drivers/ahci_block_read.sotlas","kernel/src/drivers/ahci_runtime.sotlas","kernel/src/drivers/nvme.sotlas","kernel/src/drivers/xhci_configuration.sotlas","kernel/src/drivers/xhci_context.sotlas","kernel/src/drivers/xhci_descriptor.sotlas","kernel/src/drivers/xhci_hid_context.sotlas","kernel/src/drivers/xhci_runtime.sotlas"],
            "DF-8c integration must remain limited to certified NVMe/AHCI callers plus xHCI runtime/context/descriptor/configuration and HID context ring"
        )
