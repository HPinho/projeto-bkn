#!/usr/bin/env python3
"""Guardrails do Address Device xHCI por Slot ID antes de transfers EP0."""
...
class XhciAddressTests(unittest.TestCase):
    ...
    def test_address_state_is_partitioned_by_slot_epoch(self):
        text = ADDRESS.read_text(encoding="utf-8")
        self.assertIn("XHCI_ADDRESS_READY_SLOTS", text)
        self.assertIn("XHCI_ADDRESS_EPOCHS", text)
        self.assertIn("XHCI_ADDRESS_DEVICE_ADDRESSES", text)
        self.assertIn("xhci_device_table_slot_epoch(slot_id)", text)
        self.assertIn("XHCI_DEVICE_STATE_ADDRESSED", text)
