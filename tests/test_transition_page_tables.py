...
class TransitionPageTableTests(unittest.TestCase):
    ...
    def test_composer_requires_direct_map_wx_image_framebuffer_and_guarded_stack(self):
        body = self.text.split("pub fn transition_page_tables_build", 1)[1]
        direct_pos = body.index("direct_map_build_from_boot_ranges(")
        ...
        for token in ("if !direct.valid", "if !image_map.valid", "if !framebuffer_map.valid", "if !stack_map.valid"):
    ...
    def test_framebuffer_is_identity_rw_nx_and_conservative_uc(self):
        self.assertIn("virtual_base: physical_base", self.framebuffer)
        for flag in ("X86_PTE_PRESENT", "X86_PTE_WRITABLE", "X86_PTE_NX", "X86_PTE_WRITE_THROUGH", "X86_PTE_CACHE_DISABLE"):
            self.assertIn(flag, self.framebuffer)
        self.assertNotIn("X86_PTE_USER", self.framebuffer)
    ...
    def test_result_exposes_root_physical_without_activating_it(self):
        ...
        for token in ("__write_cr3", "x86_write_cr3", "__invlpg", "ExitBootServices", "GetMemoryMap", "AllocatePages", "BootServices", "pmm_alloc_page("):
