...
class KernelSmpBaseTests(unittest.TestCase):
    def test_low_memory_page_comes_only_from_final_conventional_inventory(self):
        ...
        for token in (
            "pmm_inventory_is_valid()",
            "pmm_get_conventional_region(ordinal)",
            "LOW_MEMORY_SMP_MIN: u64 = 0x00010000",
            "LOW_MEMORY_SMP_LIMIT: u64 = 0x000A0000",
            "x86_page_align_down(candidate_limit)",
        ):
    ...
    def test_lapic_has_real_init_sipi_icr_sequence(self):
        ...
        for token in (
            "LAPIC_REG_ICR_LOW: u64 = 0x300",
            "LAPIC_REG_ICR_HIGH: u64 = 0x310",
            "LAPIC_ICR_INIT_ASSERT: u32 = 0x0000C500",
            "LAPIC_ICR_INIT_DEASSERT: u32 = 0x00008500",
            "LAPIC_ICR_STARTUP: u32 = 0x00000600",
            "LAPIC_ICR_DELIVERY_STATUS",
            "lapic_send_init_assert",
    ...
    def test_trampoline_enters_long_mode_with_nx_and_baken_cr3(self):
        ...
        for token in (
            "SMP_TRAMPOLINE_BYTES: usize = 166",
            "SMP_TRAMPOLINE_LONG_MODE_OFFSET: u64 = 0x42",
            "SMP_TRAMPOLINE_CR3_VALUE_OFFSET: usize = 0x80",
            "SMP_TRAMPOLINE_GDT_POINTER_OFFSET: usize = 0xA0",
            "root_physical > 0xFFFFFFFF",
            "smp_trampoline_write_u32(base, SMP_TRAMPOLINE_CR3_VALUE_OFFSET",
            "smp_trampoline_write_u64(base, SMP_TRAMPOLINE_IDLE_OFFSET",
            "__dma_fence();",
        ):
            self.assertIn(token, text)
    ...
    def test_standard_smoke_requires_smp_base_even_on_uniprocessor(self):
        ...
        self.assertIn("SMP_BASE_READY", REQUIRED)
