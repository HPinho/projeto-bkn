...
Write-Host "      BAKEN OS — BUILD PIPELINE (FASE 14/15)                     " -ForegroundColor Cyan
Write-Host "=================================================================" -ForegroundColor Cyan

# 0. Garante que os diretórios de build existem
Write-Host "`n[0/3] Criando diretórios de build..." -ForegroundColor Yellow
...
# 1. Compilação com GCC Nativo para Subsystem 10 (EFI Application)
...
if (-not (Test-Path $efi_src)) {
    ...
    & "$gcc_bin\gcc.exe" -Wall -Wextra -nostdlib -shared `
        "-Wl,--subsystem,10" `
...
Write-Host "      INICIANDO BAKEN OS NO QEMU                                 " -ForegroundColor Green
